...
class MixedMNIST(Dataset):
    def __init__(
        self,
        colored_rgb_mnist_dataset,
        pre_trained_ae,
        root="./datasets_bla_bla",
        train=True,
    ):
        """
        ...
        self.mnist_expanded_images = (
            self._expand_and_normalise()
        )  # it would be a torch tensor
        self.colored_mnist = (
            colored_rgb_mnist_dataset  # it would also be a torch tensor
        )
    ...
    def _labels(self):
        ...
        labels = torch.cat(
            (torch.zeros(len_colored_data // 2), torch.ones(len_colored_data // 2))
        )
