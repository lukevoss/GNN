...
def split_data_axis(data, split, direction):
    ...
    for d in range(data.shape[0]):
        if data[d][direction] <= split:
            left_data = np.append(left_data, np.reshape(
                data[d], (1, data[d].shape[0])), axis=0)
        else:
            right_data = np.append(right_data, np.reshape(
                data[d], (1, data[d].shape[0])), axis=0)
...
class RandomDensityTree:
    ...
    def fit(self, data, axis=0):
        ...
        self.rootnode = Node(data, self.cov, [], self.tree, num_splits=self.num_splits, min_infogain=self.min_infogain,
                             max_depth=self.max_depth, pointer=0, rand=self.rand, splittype=self.splittype)
        ...
        if (max_depth == 0 or data.shape[0] == 1):
            ...
            if (self.splittype == 'axis'):
                ...
                if (rand == True):
                    for dim in range(int(num_splits)):
                        ...
                        rnd_split = rnd.uniform(
                            min(data[:, direction]), max(data[:, direction]))

                        rnd_splits.append(
                            {'split': rnd_split, 'direction': direction})
                # generating non-random evenly spaced splits is also possible but only if the split is axis-aligned
                else:
                    rnd_splits = np.concatenate([np.linspace(min(data[:, 0]), max(
                        data[:, 0])), np.linspace(min(data[:, 1]), max(data[:, 1]))], axis=0)

            # random splits are generated differently
            elif (self.splittype == 'linear'):
                rnd_splits = []
                for n in range(num_splits):
                    start = np.array([rnd.uniform(min(data[:, 0]), max(
                        data[:, 0])), rnd.uniform(min(data[:, 1]), max(data[:, 1]))])
            ...
            for s in range(num_splits):
                ...
                if (self.splittype == 'linear'):
                    ...
                    left_data, right_data = split_data_axis(
                        data, rnd_splits[s]['split'], rnd_splits[s]['direction'])
                ...
                if (left_data.shape[0] > 2 and right_data.shape[0] > 2):
                    ...
                    #   print(left_data)
                    info_gains[s] = (
                        info_gain(data, self.cov, left_data, cov_l, right_data, cov_r))
            ...
            if len(info_gains) == 0:
                ...
                if info_gains[best] >= min_infogain:
                    ...
                    leftnode = Node(left_datas[best], covs_left[best], self.history, tree, self.num_splits,
                                    self.min_infogain, self.maxdepth-1, 2*pointer+1, rand=rand, splittype=self.splittype)
                    tree[2*pointer+1] = leftnode
                    self.left_child = leftnode

                    self.history[len(self.history)-1]['child'] = 'right'
                    rightnode = Node(right_datas[best], covs_right[best], self.history, tree, self.num_splits,
                                     self.min_infogain, self.maxdepth-1, 2*pointer+2, rand=rand, splittype=self.splittype)
