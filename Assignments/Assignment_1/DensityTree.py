...
def split_data_axis(data, split, direction):
    ...
    for d in range(data.shape[0]):
        if data[d][direction] <= split:
            left_data = np.append(
                left_data, np.reshape(data[d], (1, data[d].shape[0])), axis=0
            )
        else:
            right_data = np.append(
                right_data, np.reshape(data[d], (1, data[d].shape[0])), axis=0
            )
...
class RandomDensityTree:
    ...
    def fit(self, data, axis=0):
        ...
        self.rootnode = Node(
            ...
            min_infogain=self.min_infogain,
        ...
        if max_depth == 0 or data.shape[0] == 1:
            ...
            if self.splittype == "axis":
                ...
                # generating non-random evenly spaced splits is also possible but only if the split is axis-aligned
                    ...
                    rnd_splits = np.concatenate(
                        [
                            np.linspace(min(data[:, 0]), max(data[:, 0])),
                            np.linspace(min(data[:, 1]), max(data[:, 1])),
                        ],
                        axis=0,
                    )
            ...
            # random splits are generated differently
                ...
                for n in range(num_splits):
                    start = np.array(
                        [
                            rnd.uniform(min(data[:, 0]), max(data[:, 0])),
                            rnd.uniform(min(data[:, 1]), max(data[:, 1])),
                        ]
                    )
            ...
            for s in range(num_splits):
                ...
                if left_data.shape[0] > 2 and right_data.shape[0] > 2:
                    ...
                    #   print(left_data)
                    info_gains[s] = info_gain(
                        data, self.cov, left_data, cov_l, right_data, cov_r
                    )
    ...
    def predict(self, point):
        ...
        if self.isLeaf == True:
            ...
            if self.splittype == "axis":
                ...
                if (
                    np.cross(
                        (self.split + self.split_dim) - self.split, point - self.split
                    )
                    < 0
                ):
                    return self.left_child.predict(point)
